...
import type { ThreeState } from '$lib/three/three-api.store';
...
const fullscreenGlitcDecreaseTask = async ({ passes: { glitchPass } }: ThreeState) => {
	await animate({
		from: { byp: 0 },
		to: { byp: 2 },
		duration: 200,
...
const fullscreenGlitchIncreaseTask = async ({ passes: { glitchPass } }: ThreeState) => {
	await animate({
		from: { byp: 2 },
		to: { byp: 0 },
		duration: 200,
